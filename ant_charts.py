...
def plot_improved_fitness_progression():
    ...
    for pset in PARAM_SETS:
        ...
        filename = f"ant_colony_fitness_iterations_paramset_{pset}.csv"
        ...
        # Configure plot
        plt.title(f'Ant Colony Optimization - Config {pset} Fitness Progression', fontsize=14)
        ...
        # Save as PNG
        plt.savefig(os.path.join(OUTPUT_DIR, f'aco_config_{pset}_fitness.png'))
        plt.close()
...
def plot_final_fitness_distribution():
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'aco_final_fitness_distribution.png'))
    plt.close()
...
def plot_convergence_comparison():
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'aco_convergence_comparison.png'))
    plt.close()


def plot_computation_time():
    """Plot computation time analysis for ACO configurations"""
    # Read aggregated results
    agg_file = os.path.join(RESULTS_DIR, 'ant_colony_aggregated_results.csv')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'aco_computation_time.png'))
    plt.close()
...
def plot_parameter_performance():
    ...
    plt.grid(False)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'aco_performance_comparison.png'))
    plt.close()


def main():
    """Generate all ACO charts"""
    print("Generating Ant Colony Optimization charts...")
